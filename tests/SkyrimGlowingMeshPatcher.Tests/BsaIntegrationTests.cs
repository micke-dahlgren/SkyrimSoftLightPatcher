...
public sealed class BsaIntegrationTests
{
    ...
    public async Task PatchExecutor_ArchiveBackedMesh_WritesLooseOverrideToGeneratedArchive()
    {
        ...
        var destinationPath = CreateTempDirectory("bsa-patch-output");
        var outputArchivePath = Path.Combine(destinationPath, "LightingEffect1 Mesh Patcher Output.zip");
        ...
        var manifest = await patchExecutor.ExecuteAsync(report, outputArchivePath, outputRootPath);
