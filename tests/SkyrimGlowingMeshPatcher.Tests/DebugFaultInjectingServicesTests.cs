...
public sealed class DebugFaultInjectingServicesTests
{
    ...
    public async Task PatchExecutor_PreparingOutputLowDisk_ThrowsBeforeInnerExecutorRuns()
    {
        ...
        await Assert.ThrowsAsync<LowDiskSpaceException>(() => sut.ExecuteAsync(report, archivePath, CreateOutputRootPath(archivePath)));
    ...
    public async Task PatchExecutor_WritingOutputManifestLowDisk_ThrowsWhenManifestStageStarts()
    {
        ...
        var report = CreateReport();
        var archivePath = CreateArchivePath();
        var error = await Assert.ThrowsAsync<LowDiskSpaceException>(() => sut.ExecuteAsync(report, archivePath, CreateOutputRootPath(archivePath)));
    ...
    private static string CreateOutputRootPath(string archivePath)
    {
        return Path.Combine(Path.GetDirectoryName(archivePath)!, "Glowing Mesh Patcher Output");
    }
